...
def compute_statistics(
    ...
    all_score_dicts: Optional[List[List[Dict[str, float]]]] = None,
) -> Dict[str, Dict[str, float]]:
    """Compute variance statistics from per-sample aggregate values.

    Only processes keys present in per_sample_agg. Derived metrics
    (e.g. precision/recall/F1) that don't appear here are automatically excluded.

    Args:
        per_sample_agg: {score_name: [pass@1_using_rollout_0, pass@1_using_rollout_1, ...]}
        all_score_dicts: Raw per-task score dicts, needed for avg_sample_std_dev.

    Returns:
        {score_name: {"std_dev_across_runs": ..., "std_err_across_runs": ..., "avg_sample_std_dev": ...}}
    """
    result = {}

    for name, sample_means in per_sample_agg.items():
        ...
        # avg_sample_std_dev requires per-task raw data
        if all_score_dicts is not None:
            task_std_devs = []
            ...
            if task_std_devs:
                stats["avg_sample_std_dev"] = sum(task_std_devs) / len(task_std_devs)
...
class BaseMetrics(ABC):
    ...
    Tier 2 (derived aggregates): Also override _add_derived_metrics() to inject
    parallel computations (e.g. precision/recall/F1) into aggregate after standard
    aggregation. The base score composes with the standard pipeline; derived metrics
    run alongside it with different values per aggregate key.
    Example: AnswerJudgementMetrics.

    Tier 3 (fully custom): Override compute() entirely to bypass the standard pipeline.
    Return a custom MetricsOutput with domain-specific structure.
    ...
    def _add_derived_metrics(
        ...
        """Hook for subclasses to add derived aggregate metrics (e.g. precision/recall/F1).

        Called AFTER standard aggregation and statistics. Metrics added here
        will NOT get automatic statistics (since they're not in per_sample_aggregate).
        Subclasses can add different derived values per aggregate key since
        "which prediction was chosen" differs between pass@k, majority@k, etc.
        """
        pass
    ...
    def compute(self, task_results: List[List[dict]]) -> MetricsOutput:
        ...
        # majority@k and no_answer for each k value.
        ...
        if has_answers:
            for k_val in range(1, k + 1):
                ...
                # no_answer: fraction of tasks where ALL first k_val answers are NOT_FOUND
                no_answer_count = sum(
                    1 for task_answers in all_answers if all(a is NOT_FOUND for a in task_answers[:k_val])
                )
                no_answer_pct = 100.0 * no_answer_count / len(all_answers) if all_answers else 0.0
                if no_answer_pct > 0:
                    for agg_key in [f"pass@{k_val}", f"pass@1[avg-of-{k_val}]", f"majority@{k_val}"]:
                        if agg_key in aggregate:
                            aggregate[agg_key][self.no_answer_label] = no_answer_pct

        # Per-sample aggregate
        per_sample_aggregate = self._compute_per_sample_aggregate(all_score_dicts, score_names, k)

        # Fuse statistics into pass@1[avg-of-*] entries only.
        # per_sample_aggregate is a pass@1 decomposition — stats describe pass@1 variance
        # across samples, so they only belong on pass@1[avg-of-k].
        if k > 1:
            statistics = compute_statistics(per_sample_aggregate, all_score_dicts)
        ...
        # Derived metrics hook — runs after statistics so derived metrics don't get auto-stats.
        self._add_derived_metrics(aggregate, task_results, all_score_dicts, all_answers)
