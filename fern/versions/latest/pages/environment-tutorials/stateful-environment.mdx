...
This tutorial focuses on the **Resources Server** implementation for environments that maintain state across tool calls within an episode. The full workflow — task data preparation, agent/model configuration, rollout collection, and training — follows the same steps as the [single-step tutorial](/latest/environment-tutorials/single-step-environment). What changes here is the addition of per-episode session state via middleware.

<NavButton href="/latest/environment-tutorials/multi-step-environment" label="Multi-Step Environment" direction="prev" />
...
<NavButton href="/latest/environment-tutorials/real-world-environment" label="Real-World Environment" direction="next" />
