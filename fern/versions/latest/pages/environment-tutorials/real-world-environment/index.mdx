---
title: "Real-World Environment"
description: ""
position: 5
---
import { NavButton } from "../../../../../components/NavButton";

The Workplace Assistant environment simulates an office with email, calendar, analytics, project management, and CRM toolkits. It uses dynamic routing, per-session state, and state-based verification to grade outcomes.

<NavButton href="/latest/environment-tutorials/stateful-environment" label="Stateful Environment" direction="prev" />

---

<Cards>

<Card title="Generating Training Data" href="/latest/environment-tutorials/real-world-environment/generating-training-data">
Generate synthetic multi-step tool-calling data using NeMo Data Designer.
</Card>

<Card title="Resources Server Implementation" href="/latest/environment-tutorials/real-world-environment/resources-server-implementation">
Dynamic routing, per-session state, state-based verification, and rollout transcripts.
</Card>

</Cards>

---

<NavButton href="/latest/environment-tutorials" label="Back to Environment Tutorials" direction="back" />
