...
Generate synthetic task data (user queries) for the [Workplace Assistant](/latest/environment-tutorials/real-world-environment) environment using [NeMo Data Designer](https://github.com/NVIDIA-NeMo/DataDesigner). 
...
After generating your task data, use it with the Workplace Assistant resources server to [collect rollouts](/latest/get-started/rollout-collection) (where the environment produces model responses) and then proceed to [GRPO training](/latest/training-tutorials/nemo-rl-grpo).

<NavButton href="/latest/environment-tutorials/real-world-environment/resources-server-implementation" label="Resources Server Implementation" direction="next" />
