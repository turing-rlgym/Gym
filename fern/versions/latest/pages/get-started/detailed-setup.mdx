...
<NavButton href="/latest/get-started" label="Previous: Quickstart" direction="prev" />
...
Refer to [Configuration](/latest/reference/configuration) for additional `env.yaml` options.
...
- **Self-hosted models**: Use vLLM to serve your own models (see the [model-server-vllm](/latest/model-server/vllm))
