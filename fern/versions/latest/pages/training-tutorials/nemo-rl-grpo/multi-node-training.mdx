...
<NavButton href="/latest/training-tutorials/nemo-rl-grpo/single-node-training" label="Previous: Single Node Training" direction="prev" />
...
<Info>
**Complete the [Single Node Training](/latest/training-tutorials/nemo-rl-grpo/single-node-training) first. Do not skip it.** The single-node setup validates that your environment is configured correctly before attempting multi-node training.
...
<Tip>
If you are using enroot following the steps in the [Setup](/latest/training-tutorials/nemo-rl-grpo/setup) doc and downloaded the container locally, use the local container filepath instead:
