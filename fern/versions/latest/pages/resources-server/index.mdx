...
Every Resources server implements a `verify()` function that evaluates the result of a rollout and returns a reward signal for training. See [Task Verification](/latest/about/concepts/task-verification) for verification approaches, patterns, and best practices.

For semantic or rubric-based scoring, `verify()` may call a **second language model** (LLM-as-a-judge); the concept is outlined in [task-verification](/latest/about/concepts/task-verification) under *What is LLM-as-a-judge?*. For configuration, deployment, and implementation patterns, see [Llm As Judge Verification](/latest/environment-tutorials/llm-as-judge-verification).
...
<Note>
[Resources Server Fields](/latest/reference/configuration#resources-server-fields) for server configuration syntax and fields.
