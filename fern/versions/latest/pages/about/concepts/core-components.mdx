...
description: ""
---
> New to reinforcement learning for LLMs? Start with [training-approaches](/latest/about/concepts/training-approaches) for context on SFT, RL, and RLVR, or refer to [Key Terminology](/latest/about/concepts/key-terminology) for a quick glossary.
...
- **[Agent](/latest/agent-server)** servers define whether a rollout is single-step or multi-step, single-turn or multi-turn, and orchestrate the full rollout lifecycle: calling the model, routing tool calls to resources, and collecting the final reward. The Agent server does not run an LLM itself — it delegates all text generation to the Model server.
- **[Model](/latest/model-server)** servers are stateless LLM inference endpoints. They receive a conversation and return the model's next output (text, tool calls, or code) with no memory or orchestration logic.
- **[Resources](/latest/resources-server)** servers provide the tasks that agents solve, the tools and external state they interact with, and the verification logic that scores performance and returns reward signals for training. Each resources server manages isolated per-rollout state via session IDs.
