...
description: ""
---
NeMo Gym uses YAML configuration files to define [Model, Resources, and Agent servers](/latest/about/concepts/core-components). Each server gets its own configuration block, providing modular control over the entire training environment.
...
Different server types have additional required fields (e.g., `domain` for resources servers, `resources_server` and `model_server` for agents). See [Configuration](/latest/reference/configuration) for complete field specifications.
